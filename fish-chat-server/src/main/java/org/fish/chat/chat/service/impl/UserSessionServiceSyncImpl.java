...
public class UserSessionServiceSyncImpl implements UserSessionService, UserSessionListener, InitializingBean
    ...
    private void checkSession() {
        ...
        for (Map<Long, UserSession> tempUserSessionMap : userSessionMapList) {
            int userCount = 0, cleanCount = 0, cleanUserCount = 0;

            int androidUserCount = 0, androidForegroundCount = 0, androidBackgroundCount = 0, androidWaitReconnectCount = 0;

            int iosUserCount = 0, iosWaitReconnectCount = 0;
...
//                addMapCount(modelCountMap, StringUtils.defaultString(userSession.getClientModel(), "unknown"));
            ...
            LoggerManager.info(String.format("userCount = %s", userCount));
            LoggerManager.info(String.format("androidUserCount = %s", androidUserCount));
            LoggerManager.info(String.format("iosUserCount = %s", iosUserCount));
            LoggerManager.info(String.format("cleanCount = %s,", cleanCount));
...
//            }
    ...
    private String getOnlineKey(long userId) {
        return "chat_online_" + userId; //供排序使用的key
    }
